...
TEST_F(TestConstraintExpr, eq) {
	...
	if (!a->val()->eq(b->val())) {
		fprintf(stdout, "a=%lld b=%lld\n",
				static_cast<ExprValInt *>(a->val().get())->val_u(),
				static_cast<ExprValInt *>(b->val().get())->val_u());
	}
			...
			{a, b, c},
			{
				ConstraintExpr::mk(
						ExprBin::mk(
								ExprFieldRef::mk(a),
								BinOp_Ne,
								ExprValLiteral::mk(ExprValInt::ZERO))),
				ConstraintExpr::mk(
						ExprBin::mk(
								ExprFieldRef::mk(a),
								BinOp_Lt,
								ExprValLiteral::mk(ExprValInt::mk(128, 8, false)))),
				ConstraintExpr::mk(
						ExprBin::mk(
								ExprFieldRef::mk(b),
								BinOp_Ne,
				...
				ConstraintExpr::mk(
					ExprBin::mk(
							...
							ExprBin::mk(
									...
									ExprFieldRef::mk(b))))}));
	ASSERT_EQ(
			static_cast<ExprValInt *>(c->val().get())->val_u(),
			(
					static_cast<ExprValInt *>(a->val().get())->val_u() +
					static_cast<ExprValInt *>(b->val().get())->val_u()));
}
