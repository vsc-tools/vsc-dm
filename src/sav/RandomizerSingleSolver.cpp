...
#include "sav/RandomizerSingleSolver.h"

#include "sav/ExprValInt.h"
#include "sav/FieldBoundVisitor.h"
#include "sav/RandFieldCollector.h"
#include "SolveGroupSetBuilder.h"

namespace vsc {

RandomizerSingleSolver::RandomizerSingleSolver(uint32_t seed, ISolverFactory *backend) :
		m_rng(seed),
		m_backend(backend) {
	...
	for (std::vector<Field *>::const_iterator it=solvegroups->unconstrained().begin();
		...
		if (fs->is_signed()) {
			...
			if (fs->width() > 31) {
				...
				/*
				fs->val(ExprValNumericSP(new ExprValInt(
...
ExprSP RandomizerSingleSolver::create_rand_domain_constraint(
	...
	if (range_sz > 64) {
		...
		if (r_type >= 0 && r_type <= 2) { // range-based randomization
			...
			ret = ExprSP(new ExprBin(
					new ExprBin(
							...
							BinOp_Ge,
							new ExprValLiteral(
									new ExprValInt(min, 64, false))),
					BinOp_And,
					new ExprBin(
							new ExprFieldRef(field),
							BinOp_Le,
							new ExprValLiteral(
									new ExprValInt(max, 64, false)))
					));
		} else { // Single value
			ret = ExprSP(new ExprBin(
					new ExprFieldRef(field),
					BinOp_Eq,
					new ExprValLiteral(
							new ExprValInt(single_val, 32, false))
					));
		}
	} else {
		...
		ret = ExprSP(new ExprBin(
				...
				BinOp_Eq,
				new ExprValLiteral(
						new ExprValInt(target_val, 32, false))
				));
	}
